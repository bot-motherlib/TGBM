...
struct ReplyKeyboardMarkup {
   ...
   * that are @mentioned in the text of the Message object; 2) if the bot's message is a reply to a message in
   * the same chat and forum topic, sender of the original message.Example: A user requests to change the
   * bot's language, bot replies to the request with a keyboard to select the new language. Other users in the
   * group don't see the keyboard. */
  optional<bool> selective;

  consteval static bool is_mandatory_field(std::string_view name) {
    return string_switch<bool>(name).case_("keyboard", true).or_default(false);
  }

  bool operator==(const ReplyKeyboardMarkup&) const;
  std::strong_ordering operator<=>(const ReplyKeyboardMarkup&) const;
};
