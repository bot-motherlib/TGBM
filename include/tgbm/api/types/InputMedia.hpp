...
struct InputMedia {
  oneof<InputMediaAnimation, InputMediaAudio, InputMediaDocument, InputMediaLivePhoto, InputMediaPhoto,
        InputMediaVideo>
      data;
  ...
  InputMediaLivePhoto* get_livephoto() noexcept {
    return data.get_if<InputMediaLivePhoto>();
  }
  const InputMediaLivePhoto* get_livephoto() const noexcept {
    return data.get_if<InputMediaLivePhoto>();
  }
