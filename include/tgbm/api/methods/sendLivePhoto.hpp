...
struct send_live_photo_request {
   ...
   * Pass a file_id as String to send a video that exists on the Telegram servers (recommended) or upload a
   * new video using multipart/form-data. More information on Sending Files ». Sending live photos by a URL is
   * currently unsupported. */
  file_or_str live_photo;
  ...
  static constexpr std::string_view api_method_name = "sendLivePhoto";
  ...
  void fill_nonfile_args(auto& body) const {
    ...
    if (const auto* str = live_photo.get_str())
      body.arg("live_photo", *str);
  ...
  void fill_file_args(application_multipart_form_data& body) const {
    if (const InputFile* f = live_photo.get_file())
      body.arg("live_photo", *f);
