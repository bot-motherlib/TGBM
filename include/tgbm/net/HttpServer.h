...
class HttpServer {
 ...
 public:
  typedef std::function<std::string(const std::string&, const std::unordered_map<std::string, std::string>&)>
      ServerHandler;
 ...
 protected:
  class Connection : public std::enable_shared_from_this<Connection> {
   ...
   protected:
    ...
    void _readHeader() {
      ...
      boost::asio::async_read_until(
          _socket, *data, "\r\n\r\n", [self, data](const boost::system::error_code& e, std::size_t n) {
            ...
            if (size == 0) {
              std::string answer =
                  self->_httpParser.generateResponse("Bad request", "text/plain", 400, "Bad request", false);
              boost::asio::async_write(self->_socket, boost::asio::buffer(answer),
    ...
    void _readBody(std::shared_ptr<boost::asio::streambuf> data, unsigned long long size,
      ...
      boost::asio::async_read(
          _socket, *data, boost::asio::transfer_exactly(size - data->size()),
          [self, data, size, headers](const boost::system::error_code& e, std::size_t) {
            ...
            try {
              ...
              answer = self->_httpParser.generateResponse("Internal server error", "text/plain", 500,
