...
class CallbackQuery {
 public:
  ...
  /**
   * @brief Optional. Message with the callback button that originated the query. Note that message content
  ...
  /**
   * @brief Global identifier, uniquely corresponding to the chat to which the message with the callback
   * button was sent. Useful for high scores in games.
   */
  std::string chatInstance;

  /**
   * @brief Data associated with the callback button. Be aware that a bad client can send arbitrary data in
   * this field.
   */
  std::string data;
