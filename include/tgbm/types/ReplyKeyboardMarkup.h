 ...
 * reply options (see [Introduction to bots](https://core.telegram.org/bots/features#keyboards) for details
 * and examples).
 *
 * @ingroup types
 */
class ReplyKeyboardMarkup : public GenericReply {
 public:
  ...
  /**
   * @brief Optional. Requests clients to resize the keyboard vertically for optimal fit (e.g., make the
   * keyboard smaller if there are just two rows of buttons).
   *
   * Defaults to false, in which case the custom keyboard is always of the same height as the app's standard
   * keyboard.
   */
  bool resizeKeyboard;
  ...
  /**
   * @brief Optional. The placeholder to be shown in the input field when the keyboard is active; 1-64
   * characters
   */
   ...
   * reply to a message in the same chat and forum topic, sender of the original message. Example: A user
