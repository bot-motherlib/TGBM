...
class ReplyParameters {
 public:
  ...
  /**
   * @brief Identifier of the message that will be replied to in the current chat, or in the chat chatId if it
   * is specified
   */
  std::int32_t messageId;

  /**
   * @brief Optional. If the message to be replied to is from a different chat, unique identifier for the chat
  ...
  /**
   * @brief Optional. Pass True if the message should be sent even if the specified message to be replied to
   * is not found.
   *
   ...
   * strikethrough, spoiler, and customEmoji entities. The message will fail to send if the quote isn't found
