...
class WebhookInfo {
 public:
  ...
  /**
   * @brief Optional. Unix time for the most recent error that happened when trying to deliver an update via
   * webhook
   */
  std::int32_t lastErrorDate;

  /**
   * @brief Optional. Error message in human-readable format for the most recent error that happened when
   * trying to deliver an update via webhook
   */
  std::string lastErrorMessage;

  /**
   * @brief Optional. Unix time of the most recent error that happened when trying to synchronize available
   * updates with Telegram datacenters
   */
  std::int32_t lastSynchronizationErrorDate;

  /**
   * @brief Optional. Maximum allowed number of simultaneous HTTPS connections to the webhook for update
   * delivery
   */
  std::int32_t maxConnections;

  /**
   * @brief Optional. A list of update types the bot is subscribed to. Defaults to all update types except
   * chatMember
   */
