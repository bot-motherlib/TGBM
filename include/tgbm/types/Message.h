...
class Message {
 public:
   ...
   *
   * For backward compatibility, the field contains a fake sender user in non-channel chats, if the message
   ...
   * For backward compatibility, the field from contains a fake sender user in non-channel chats, if the
  ...
  /**
   * @brief Optional. True, if the message is a channel post that was automatically forwarded to the connected
   * discussion group
   */
   ...
   *
   * Note that the Message object in this field will not contain further replyToMessage fields even if it
   * itself is a reply.
   */
  Message::Ptr replyToMessage;

  /**
   * @brief Optional. Information about the message that is being replied to, which may come from another chat
   * or forum topic
   */
  ...
  /**
   * @brief Optional. True, if the message was sent by an implicit action, for example, as an away or a
  ...
  /**
   * @brief Optional. Signature of the post author for messages in channels, or the custom title of an
  ...
  /**
   * @brief Optional. For text messages, special entities like usernames, URLs, bot commands, etc. that appear
   * in the text
   */
  std::vector<MessageEntity::Ptr> entities;

  /**
   * @brief Optional. Options used for link preview generation for the message, if it is a text message and
  ...
  /**
   * @brief Optional. For messages with a caption, special entities like usernames, URLs, bot commands, etc.
  ...
  /**
   * @brief Optional. New members that were added to the group or supergroup and information about them (the
   * bot itself may be one of these members)
   */
  std::vector<User::Ptr> newChatMembers;

  /**
   * @brief Optional. A member was removed from the group, information about them (this member may be the bot
   * itself)
   */
   ...
   * supergroup when it is created. It can only be found in replyToMessage if someone replies to a very first
   ...
   *
   * This field can't be received in a message coming through updates, because bot can't be a member of a
   * channel when it is created. It can only be found in replyToMessage if someone replies to a very first
  ...
  /**
   * @brief Optional. Service message: the user allowed the bot to write messages after adding it to the
   * attachment or side menu, launching a Web App from a link, or accepting an explicit request from a Web App
   * sent by the method [requestWriteAccess](https://core.telegram.org/bots/webapps#initializing-mini-apps)
   */
