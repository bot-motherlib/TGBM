...
class TGBOT_API Api {
 ...
 public:
   ...
   * earliest unconfirmed update are returned. An update is considered confirmed as soon as Api::getUpdates is
   * called with an offset higher than its updateId. The negative offset can be specified to retrieve updates
   ...
   * all update types except chat_member, message_reaction, and message_reaction_count (default). If not
   * specified, the previous setting will be used. Please note that this parameter doesn't affect updates
   * created before the call to the getUpdates, so unwanted updates may be received for a short period of
   * time.
   *
   ...
   * secretToken. If specified, the request will contain a header “X-Telegram-Bot-Api-Secret-Token” with the
   ...
   *
   * If you're having any trouble setting up webhooks, please check out [this amazing guide to
   * webhooks](https://core.telegram.org/bots/webhooks).
   *
   * @param url HTTPS URL to send updates to. Use an empty string to remove webhook integration
   * @param certificate Optional. Upload your public key certificate so that the root certificate in use can
   * be checked. See our [self-signed guide](https://core.telegram.org/bots/self-signed) for details.
   * @param maxConnections Optional. The maximum allowed number of simultaneous HTTPS connections to the
   ...
   * time.
   * @param ipAddress Optional. The fixed IP address which will be used to send webhook requests instead of
   * the IP address resolved through DNS
   * @param dropPendingUpdates Optional. Pass True to drop all pending updates
   * @param secretToken Optional. A secret token to be sent in a header “X-Telegram-Bot-Api-Secret-Token” in
   ...
   * receive updates. After a successful call, you can immediately log in on a local server, but will not be
   * able to log in back to the cloud Bot API server for 10 minutes. Requires no parameters.
   *
   * @return Returns True on success.
   */
   ...
   * keyboard](https://core.telegram.org/bots/features#keyboards), instructions to remove a reply keyboard or
   * to force a reply from the user. Not supported for messages sent on behalf of a business account
   ...
   * [silently](https://telegram.org/blog/channels-2-0#silent-messages). Users will receive a notification
   * with no sound.
   * @param entities Optional. A JSON-serialized list of special entities that appear in message text, which
   * can be specified instead of parseMode
   * @param messageThreadId Optional. Unique identifier for the target message thread (topic) of the forum;
   ...
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param fromChatId Unique identifier for the chat where the original message was sent (or channel username
   ...
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param fromChatId Unique identifier for the chat where the original messages were sent (or channel
   * username in the format @channelusername)
   * @param messageIds A JSON-serialized list of 1-100 identifiers of messages in the chat fromChatId to
   ...
   * The method is analogous to the method Api::forwardMessage, but the copied message doesn't have a link to
   ...
   * in the format @channelusername)
   * @param messageId Message identifier in the chat specified in fromChatId
   * @param caption Optional. New caption for media, 0-1024 characters after entities parsing. If not
   * specified, the original caption is kept
   * @param parseMode Optional. Mode for parsing entities in the new caption. See [formatting
   * options](https://core.telegram.org/bots/api#formatting-options) for more details.
   * @param captionEntities Optional. A JSON-serialized list of special entities that appear in the new
   ...
   * @param replyMarkup Optional. Additional interface options. A JSON-serialized object for an [inline
   * keyboard](https://core.telegram.org/bots/features#inline-keyboards), [custom reply
   * keyboard](https://core.telegram.org/bots/features#keyboards), instructions to remove reply keyboard or to
   ...
   * Service messages, giveaway messages, giveaway winners messages, and invoice messages can't be copied.
   * A quiz Poll can be copied only if the value of the field correctOptionId is known to the bot.
   * The method is analogous to the method Api::forwardMessages, but the copied messages don't have a link to
   * the original message. Album grouping is kept for copied messages.
   *
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param fromChatId Unique identifier for the chat where the original messages were sent (or channel
   * username in the format @channelusername)
   * @param messageIds A JSON-serialized list of 1-100 identifiers of messages in the chat fromChatId to copy.
   ...
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param photo Photo to send. Pass a fileId as String to send a photo that exists on the Telegram servers
   ...
   * @param caption Optional. Photo caption (may also be used when resending photos by fileId), 0-1024
   ...
   * keyboard](https://core.telegram.org/bots/features#keyboards), instructions to remove a reply keyboard or
   * to force a reply from the user. Not supported for messages sent on behalf of a business account
   * @param parseMode Optional. Mode for parsing entities in the photo caption. See
   * https://core.telegram.org/bots/api#formatting-options for more details.
   * @param disableNotification Optional. Sends the message silently. Users will receive a notification with
   * no sound.
   * @param captionEntities Optional. A JSON-serialized list of special entities that appear in the caption,
  ...
  /**
   * @brief Use this method to send audio files, if you want Telegram clients to display them in the music
   * player.
   *
   ...
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param audio Audio file to send. Pass a fileId as String to send an audio file that exists on the
   * Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get an audio file from the
   * Internet, or upload a new one using multipart/form-data. https://core.telegram.org/bots/api#sending-files
   ...
   * @param thumbnail Optional. Thumbnail of the file sent; can be ignored if thumbnail generation for the
   ...
   * <file_attach_name>. https://core.telegram.org/bots/api#sending-files
   * @param replyParameters Optional. Description of the message to reply to
   * @param replyMarkup Optional. Additional interface options. A JSON-serialized object for an [inline
   * keyboard](https://core.telegram.org/bots/features#inline-keyboards), [custom reply
   * keyboard](https://core.telegram.org/bots/features#keyboards), instructions to remove a reply keyboard or
   * to force a reply from the user. Not supported for messages sent on behalf of a business account
   * @param parseMode Optional. Mode for parsing entities in the audio caption. See
   * https://core.telegram.org/bots/api#formatting-options for more details.
   * @param disableNotification Optional. Sends the message silently. Users will receive a notification with
   * no sound.
   * @param captionEntities Optional. A JSON-serialized list of special entities that appear in the caption,
   ...
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param document File to send. Pass a fileId as String to send a file that exists on the Telegram servers
   * (recommended), pass an HTTP URL as a String for Telegram to get a file from the Internet, or upload a new
   * one using multipart/form-data. https://core.telegram.org/bots/api#sending-files
   * @param thumbnail Optional. Thumbnail of the file sent; can be ignored if thumbnail generation for the
   * file is supported server-side. The thumbnail should be in JPEG format and less than 200 kB in size. A
   * thumbnail's width and height should not exceed 320. Ignored if the file is not uploaded using
   * multipart/form-data. Thumbnails can't be reused and can be only uploaded as a new file, so you can pass
   * “attach://<file_attach_name>” if the thumbnail was uploaded using multipart/form-data under
   * <file_attach_name>. https://core.telegram.org/bots/api#sending-files
   * @param caption Optional. Document caption (may also be used when resending documents by fileId), 0-1024
   * characters after entities parsing
   * @param replyParameters Optional. Description of the message to reply to
   * @param replyMarkup Optional. Additional interface options. A JSON-serialized object for an [inline
   * keyboard](https://core.telegram.org/bots/features#inline-keyboards), [custom reply
   * keyboard](https://core.telegram.org/bots/features#keyboards), instructions to remove a reply keyboard or
   * to force a reply from the user. Not supported for messages sent on behalf of a business account
   * @param parseMode Optional. Mode for parsing entities in the document caption. See
   * https://core.telegram.org/bots/api#formatting-options for more details.
   * @param disableNotification Optional. Sends the message silently. Users will receive a notification with
   * no sound.
   * @param captionEntities Optional. A JSON-serialized list of special entities that appear in the caption,
   * which can be specified instead of parseMode
   * @param disableContentTypeDetection Optional. Disables automatic server-side content type detection for
  ...
  /**
   * @brief Use this method to send video files, Telegram clients support MPEG4 videos (other formats may be
   * sent as Document).
   *
   ...
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param video Video to send. Pass a fileId as String to send a video that exists on the Telegram servers
   * (recommended), pass an HTTP URL as a String for Telegram to get a video from the Internet, or upload a
   * new video using multipart/form-data. https://core.telegram.org/bots/api#sending-files
   ...
   * “attach://<file_attach_name>” if the thumbnail was uploaded using multipart/form-data under
   * <file_attach_name>. https://core.telegram.org/bots/api#sending-files
   * @param caption Optional. Video caption (may also be used when resending videos by fileId), 0-1024
   * characters after entities parsing
   * @param replyParameters Optional. Description of the message to reply to
   * @param replyMarkup Optional. Additional interface options. A JSON-serialized object for an [inline
   * keyboard](https://core.telegram.org/bots/features#inline-keyboards), [custom reply
   * keyboard](https://core.telegram.org/bots/features#keyboards), instructions to remove a reply keyboard or
   * to force a reply from the user. Not supported for messages sent on behalf of a business account
   * @param parseMode Optional. Mode for parsing entities in the video caption. See
   * https://core.telegram.org/bots/api#formatting-options for more details.
   * @param disableNotification Optional. Sends the message silently. Users will receive a notification with
   * no sound.
   * @param captionEntities Optional. A JSON-serialized list of special entities that appear in the caption,
   ...
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param animation Animation to send. Pass a fileId as String to send an animation that exists on the
   ...
   * “attach://<file_attach_name>” if the thumbnail was uploaded using multipart/form-data under
   * <file_attach_name>. https://core.telegram.org/bots/api#sending-files
   * @param caption Optional. Animation caption (may also be used when resending animation by fileId), 0-1024
   * characters after entities parsing
   * @param replyParameters Optional. Description of the message to reply to
   * @param replyMarkup Optional. Additional interface options. A JSON-serialized object for an [inline
   * keyboard](https://core.telegram.org/bots/features#inline-keyboards), [custom reply
   * keyboard](https://core.telegram.org/bots/features#keyboards), instructions to remove a reply keyboard or
   * to force a reply from the user. Not supported for messages sent on behalf of a business account
   * @param parseMode Optional. Mode for parsing entities in the animation caption. See
   * https://core.telegram.org/bots/api#formatting-options for more details.
   * @param disableNotification Optional. Sends the message silently. Users will receive a notification with
   * no sound.
   * @param captionEntities Optional. A JSON-serialized list of special entities that appear in the caption,
  ...
  /**
   * @brief Use this method to send audio files, if you want Telegram clients to display the file as a
   * playable voice message.
   *
   * For this to work, your audio must be in an .OGG file encoded with OPUS (other formats may be sent as
   * Audio or Document). Bots can currently send voice messages of up to 50 MB in size, this limit may be
   * changed in the future.
   *
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param voice Audio file to send. Pass a fileId as String to send a file that exists on the Telegram
   * servers (recommended), pass an HTTP URL as a String for Telegram to get a file from the Internet, or
   * upload a new one using multipart/form-data. https://core.telegram.org/bots/api#sending-files
   ...
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param videoNote Video note to send. Pass a fileId as String to send a video note that exists on the
   ...
   * @param livePeriod Optional. Period in seconds for which the location will be updated (see
   ...
   * @param disableNotification Optional. Sends the message silently. Users will receive a notification with
   * no sound.
   * @param horizontalAccuracy Optional. The radius of uncertainty for the location, measured in meters;
   * 0-1500
   * @param heading Optional. For live locations, a direction in which the user is moving, in degrees. Must be
   * between 1 and 360 if specified.
   * @param proximityAlertRadius Optional. For live locations, a maximum distance for proximity alerts about
   ...
   *
   * A location can be edited until its livePeriod expires or editing is explicitly disabled by a call to
   * Api::stopMessageLiveLocation.
   *
   * @param latitude Latitude of new location
   * @param longitude Longitude of new location
   * @param chatId Optional. Required if inlineMessageId is not specified. Unique identifier for the target
   * chat or username of the target channel (in the format @channelusername)
   * @param messageId Optional. Required if inlineMessageId is not specified. Identifier of the message to
   * edit
   * @param inlineMessageId Optional. Required if chatId and messageId are not specified. Identifier of the
   * inline message
   * @param replyMarkup Optional. A JSON-serialized object for a new inline keyboard.
   * @param horizontalAccuracy Optional. The radius of uncertainty for the location, measured in meters;
   * 0-1500
   * @param heading Optional. Direction in which the user is moving, in degrees. Must be between 1 and 360 if
   * specified.
   * @param proximityAlertRadius Optional. The maximum distance for proximity alerts about approaching another
   ...
   * @param chatId Optional. Required if inlineMessageId is not specified. Unique identifier for the target
   * chat or username of the target channel (in the format @channelusername)
   * @param messageId Optional. Required if inlineMessageId is not specified. Identifier of the message with
   ...
   * @param allowsMultipleAnswers Optional. True, if the poll allows multiple answers, ignored for polls in
   * quiz mode, defaults to False
   * @param correctOptionId Optional. 0-based identifier of the correct answer option, required for polls in
   * quiz mode
   * @param explanation Optional. Text that is shown when a user chooses an incorrect answer or taps on the
   * lamp icon in a quiz-style poll, 0-200 characters with at most 2 line feeds after entities parsing
   * @param explanationParseMode Optional. Mode for parsing entities in the explanation. See
   * https://core.telegram.org/bots/api#formatting-options for more details.
   * @param explanationEntities Optional. List of special entities that appear in the poll explanation, which
   * can be specified instead of parseMode
   * @param openPeriod Optional. Amount of time in seconds the poll will be active after creation, 5-600.
   ...
   * @param isClosed Optional. Pass True, if the poll needs to be immediately closed. This can be useful for
   * poll preview.
   * @param messageThreadId Optional. Unique identifier for the target message thread (topic) of the forum;
   * for forum supergroups only
   * @param protectContent Optional. Protects the contents of the sent message from forwarding and saving
   ...
   * keyboard](https://core.telegram.org/bots/features#keyboards), instructions to remove a reply keyboard or
   * to force a reply from the user. Not supported for messages sent on behalf of a business account
   * @param emoji Optional. Emoji on which the dice throw animation is based. Currently, must be one of “🎲”,
   ...
   * as messages in the channel.
   *
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param messageId Identifier of the target message. If the message belongs to a media group, the reaction
   * is set to the first non-deleted message in the group instead.
   * @param reaction Optional. A JSON-serialized list of reaction types to set on the message. Currently, as
   ...
   * Api::sendChatAction with action = upload_photo. The user will see a “sending photo” status for the bot.
   *
   ...
   * returned.
   * @param limit Optional. Limits the number of photos to be retrieved. Values between 1-100 are accepted.
   * Defaults to 100.
   *
   ...
   * https://api.telegram.org/file/bot<token>/<filePath>, where filePath is taken from the response. It is
   * guaranteed that the filePath will be valid for at least 1 hour. When the link expires, a new one can be
   * requested by calling Api::getFile again.
   *
   ...
   * using invite links, etc., unless unbanned first. The bot must be an administrator in the chat for this to
   * work and must have the appropriate administrator rights.
   *
   * @param chatId Unique identifier for the target group or username of the target supergroup or channel (in
   * the format @channelusername)
   * @param userId Unique identifier of the target user
   * @param untilDate Optional. Date when the user will be unbanned, unix time. If user is banned for more
   * than 366 days or less than 30 seconds from the current time they are considered to be banned forever.
   * Applied for supergroups and channels only.
   * @param revokeMessages Optional. Pass True to delete all messages from the chat for the user that is being
   ...
   * By default, this method guarantees that after the call the user is not a member of the chat, but will be
   * able to join it. So if the user is a member of the chat they will also be removed from the chat. If you
   ...
   *
   * The bot must be an administrator in the supergroup for this to work and must have the appropriate
   * administrator rights. Pass True for all permissions to lift restrictions from a user.
   *
   * @param chatId Unique identifier for the target chat or username of the target supergroup (in the format
   * @supergroupusername)
   * @param userId Unique identifier of the target user
   * @param permissions A JSON-serialized object for new user permissions
   * @param untilDate Optional. Date when restrictions will be lifted for the user; Unix time. If user is
   * restricted for more than 366 days or less than 30 seconds from the current time, they are considered to
   * be restricted forever
   * @param useIndependentChatPermissions Optional. Pass True if chat permissions are set independently.
   * Otherwise, the canSendOtherMessages and canAddWebPagPreviews permissions will imply the canSendMessages,
   * canSendAudios, canSendDocuments, canSendPhotos, canSendVideos, canSendVideoNotes, and canSendVoiceNotes
   ...
   * @channelusername)
   * @param userId Unique identifier of the target user
   * @param canChangeInfo Optional. Pass True if the administrator can change chat title, photo and other
   * settings
   * @param canPostMessages Optional. Pass True if the administrator can post messages in the channel, or
   * access channel statistics; for channels only
   * @param canEditMessages Optional. Pass True if the administrator can edit messages of other users and can
   ...
   * @param canPromoteMembers Optional. Pass True if the administrator can add new administrators with a
   * subset of their own privileges or demote administrators that they have promoted, directly or indirectly
   * (promoted by administrators that were appointed by him)
   * @param isAnonymous Optional. Pass True if the administrator's presence in the chat is hidden
   * @param canManageChat Optional. Pass True if the administrator can access the chat event log, get boost
   * list, see hidden supergroup and channel members, report spam messages and ignore slow mode. Implied by
   * any other administrator privilege.
   * @param canManageVideoChats Optional. Pass True if the administrator can manage video chats
   * @param canRestrictMembers Optional. Pass True if the administrator can restrict, ban or unban chat
   * members, or access supergroup statistics
   * @param canManageTopics Optional. Pass True if the user is allowed to create, rename, close, and reopen
   ...
   *
   * Until the chat is unbanned, the owner of the banned chat won't be able to send messages on behalf of any
   * of their channels. The bot must be an administrator in the supergroup or channel for this to work and
   ...
   * canRestrictMembers administrator rights.
   *
   * @param chatId Unique identifier for the target chat or username of the target supergroup (in the format
   * @supergroupusername)
   ...
   * If you want your bot to work with invite links, it will need to generate its own link using
   * Api::exportChatInviteLink or by calling the Api::getChat method. If your bot needs to generate a new
   * primary invite link replacing its previous one, use Api::exportChatInviteLink again.
   *
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   *
   ...
   *
   * The bot must be an administrator in the chat for this to work and must have the appropriate administrator
   * rights. The link can be revoked using the method Api::revokeChatInviteLink.
   *
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param expireDate Optional. Point in time (Unix timestamp) when the link will expire
   * @param memberLimit Optional. The maximum number of users that can be members of the chat simultaneously
   * after joining the chat via this invite link; 1-99999
   * @param name Optional. Invite link name; 0-32 characters
   * @param createsJoinRequest Optional. True, if users joining the chat via the link need to be approved by
   * chat administrators. If True, memberLimit can't be specified
   *
   ...
   * @param expireDate Optional. Point in time (Unix timestamp) when the link will expire
   * @param memberLimit Optional. Maximum number of users that can be members of the chat simultaneously after
   ...
   * rights.
   *
   * @param chatId Unique identifier of the target chat or username of the target channel (in the format
   * @channelusername)
   ...
   *
   * The bot must be an administrator in the chat for this to work and must have the canInviteUsers
   * administrator right.
   *
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   ...
   * have the 'canPinMessages' administrator right in a supergroup or 'canEditMessages' administrator right in
   * a channel.
   *
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param messageId Identifier of a message to pin
   * @param disableNotification Optional. Pass True if it is not necessary to send a notification to all chat
   ...
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param messageId Optional. Identifier of a message to unpin. If not specified, the most recent pinned
   ...
   *
   * @param chatId Unique identifier for the target chat or username of the target supergroup or channel (in
   * the format @channelusername)
   *
   * @return Returns True on success.
   */
   ...
   *
   * The bot must be an administrator in the chat for this to work and must have the appropriate administrator
   * rights. Use the field canSetStickerSet optionally returned in Api::getChat requests to check if the bot
   * can use this method.
   *
   * @param chatId Unique identifier for the target chat or username of the target supergroup (in the format
   * @supergroupusername)
   ...
   *
   * The bot must be an administrator in the chat for this to work and must have the appropriate administrator
   * rights. Use the field canSetSticker_set optionally returned in Api::getChat requests to check if the bot
   * can use this method.
   *
   * @param chatId Unique identifier for the target chat or username of the target supergroup (in the format
   * @supergroupusername)
   ...
   *
   * The bot must be an administrator in the chat for this to work and must have the canManageTopics
   * administrator rights.
   *
   * @param chatId Unique identifier for the target chat or username of the target supergroup (in the format
   * @supergroupusername)
   * @param name Topic name, 1-128 characters
   * @param iconColor Optional. Color of the topic icon in RGB format. Currently, must be one of 7322096
   * (0x6FB9F0), 16766590 (0xFFD67E), 13338331 (0xCB86DB), 9367192 (0x8EEE98), 16749490 (0xFF93B2), or
   * 16478047 (0xFB6F5F)
   * @param iconCustomEmojiId Optional. Unique identifier of the custom emoji shown as the topic icon. Use
   * Api::getForumTopicIconStickers to get all allowed custom emoji identifiers.
   *
   ...
   *
   * The bot must be an administrator in the chat for this to work and must have canManageTopics administrator
   * rights, unless it is the creator of the topic.
   *
   * @param chatId Unique identifier for the target chat or username of the target supergroup (in the format
   * @supergroupusername)
   * @param messageThreadId Unique identifier for the target message thread of the forum topic
   * @param name Optional. New topic name, 0-128 characters. If not specified or empty, the current name of
   * the topic will be kept
   * @param iconCustomEmojiId Optional. New unique identifier of the custom emoji shown as the topic icon. Use
   * Api::getForumTopicIconStickers to get all allowed custom emoji identifiers. Pass an empty string to
   ...
   *
   * The bot must be an administrator in the chat for this to work and must have the canPinMessages
   ...
   * accepted the conditions via @BotFather, specify the URL that opens your game - note that this will only
   * work if the query comes from an InlineKeyboardButton button.
   * @param cacheTime Optional. The maximum amount of time in seconds that the result of the callback query
   ...
   *
   * @param commands A JSON-serialized list of bot commands to be set as the list of the bot's commands. At
   * most 100 commands can be specified.
   * @param scope Optional. A JSON-serialized object, describing scope of users for which the commands are
   * relevant. Defaults to BotCommandScopeDefault.
   * @param languageCode Optional. A two-letter ISO 639-1 language code. If empty, commands will be applied to
   ...
   * language.
   *
   * @param scope Optional. A JSON-serialized object, describing scope of users. Defaults to
   ...
   *
   * @param name Optional. New bot name; 0-64 characters. Pass an empty string to remove the dedicated name
   * for the given language.
   * @param languageCode Optional. A two-letter ISO 639-1 language code. If empty, the name will be shown to
   ...
   * chat is empty.
   *
   * @param description Optional. New bot description; 0-512 characters. Pass an empty string to remove the
   * dedicated description for the given language.
   * @param languageCode Optional. A two-letter ISO 639-1 language code. If empty, the description will be
   ...
   *
   * @param shortDescription Optional. New short description for the bot; 0-120 characters. Pass an empty
   * string to remove the dedicated short description for the given language.
   * @param languageCode Optional. A two-letter ISO 639-1 language code. If empty, the short description will
   ...
   *
   * @param chatId Optional. Unique identifier for the target private chat. If not specified, default bot's
   * menu button will be changed
   * @param menuButton Optional. A JSON-serialized object for the bot's new menu button. Defaults to
   * MenuButtonDefault
   *
   * @return Returns True on success.
   */
   ...
   * an administrator to groups or channels.
   *
   * These rights will be suggested to users, but they are free to modify the list before adding the bot.
   *
   * @param rights Optional. A JSON-serialized object describing new default administrator rights. If not
   * specified, the default administrator rights will be cleared.
   * @param forChannels Optional. Pass True to change the default administrator rights of the bot in channels.
   ...
   *
   * @param forChannels Optional. Pass True to get default administrator rights of the bot in channels.
   ...
   * @param inlineMessageId Optional.	Required if chatId and messageId are not specified. Identifier of the
   * inline message
   * @param parseMode Optional. Mode for parsing entities in the message text. See
   * https://core.telegram.org/bots/api#formatting-options for more details.
   * @param linkPreviewOptions Optional. Link preview generation options for the message
   * @param replyMarkup Optional. A JSON-serialized object for an inline keyboard.
   * @param entities Optional. List of special entities that appear in message text, which can be specified
   ...
   * inline message
   * @param replyMarkup Optional. A JSON-serialized object for an inline keyboard.
   * @param parseMode Optional. Mode for parsing entities in the message caption. See
   * https://core.telegram.org/bots/api#formatting-options for more details.
   * @param captionEntities Optional. List of special entities that appear in the caption, which can be
   ...
   * a document for document albums and to a photo or a video otherwise. When an inline message is edited, a
   * new file can't be uploaded; use a previously uploaded file via its fileId or specify a URL.
   *
   ...
   * - If the bot has canDeleteMessages permission in a supergroup or a channel, it can delete any message
   * there.
   *
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   ...
   * Api::deleteMessage for limitations on which messages can be deleted
   *
   * @return Returns True on success.
   */
   ...
   * .TGS, or [video](https://telegram.org/blog/video-stickers-better-reactions) .WEBM stickers.
   *
   * @param chatId Unique identifier for the target chat or username of the target channel (in the format
   * @channelusername)
   * @param sticker Sticker to send. Pass a fileId as String to send a file that exists on the Telegram
   ...
   * Sending Files »](https://core.telegram.org/bots/api#sending-files). Video and animated stickers can't be
   ...
   *
   * @param customEmojiIds A JSON-serialized list of custom emoji identifiers. At most 200 custom emoji
   ...
   * Api::addStickerToSet, or Api::replaceStickerInSet methods (the file can be used multiple times).
   *
   ...
   * @param stickerType Optional. Type of stickers in the set, pass “regular”, “mask”, or “custom_emoji”. By
   * default, a regular sticker set is created.
   * @param needsRepainting Optional. Pass True if stickers in the sticker set must be repainted to the color
   * of text when used in messages, the accent color if used as emoji status, white on chat photos, or another
   ...
   * @param sticker A JSON-serialized object with information about the added sticker. If exactly the same
   ...
   *
   * The method is equivalent to calling Api::deleteStickerFromSet, then Api::addStickerToSet, then
   ...
   *
   * @param sticker File identifier of the sticker
   * @param keywords Optional. A JSON-serialized list of 0-20 search keywords for the sticker with total
   ...
   * @param name Sticker set name
   * @param userId User identifier of the sticker set owner
   * @param format Format of the thumbnail, must be one of “static” for a .WEBP or .PNG image, “animated” for
   * a .TGS animation, or “video” for a WEBM video
   * @param thumbnail Optional. A .WEBP or .PNG image with the thumbnail, must be up to 128 kilobytes in size
   * and have a width and height of exactly 100px, or a .TGS animation with a thumbnail up to 32 kilobytes in
   * size (see https://core.telegram.org/stickers#animated-sticker-requirements for animated sticker technical
   ...
   * Pass a fileId as a String to send a file that already exists on the Telegram servers, pass an HTTP URL as
   * a String for Telegram to get a file from the Internet, or upload a new one using multipart/form-data.
   ...
   *
   * @param name Sticker set name
   * @param customEmojiId Optional. Custom emoji identifier of a sticker from the sticker set; pass an empty
   ...
   * @param cacheTime Optional. The maximum amount of time in seconds that the result of the inline query may
   * be cached on the server. Defaults to 300.
   * @param isPersonal Optional. Pass True if results may be cached on the server side only for the user that
   * sent the query. By default, results may be returned to any user who sends the same query.
   * @param nextOffset Optional. Pass the offset that a client should send in the next query with the same
   * text to receive more results. Pass an empty string if there are no more results or if you don't support
   * pagination. Offset length can't exceed 64 bytes.
   * @param button Optional. A JSON-serialized object describing a button to be shown above inline query
   * results
   *
   * @return On success, True is returned.
   */
   ...
   * @param payload Bot-defined invoice payload, 1-128 bytes. This will not be displayed to the user, use for
   ...
   * @param providerData Optional. A JSON-serialized data about the invoice, which will be shared with the
   * payment provider. A detailed description of required fields should be provided by the payment provider.
   * @param photoUrl Optional. URL of the product photo for the invoice. Can be a photo of the goods or a
   ...
   * @param needShippingAddress Optional. Pass True, if you require the user's shipping address to complete
   * the order
   ...
   * @param replyParameters Optional. Description of the message to reply to
   * @param replyMarkup Optional. A JSON-serialized object for an inline keyboard. If empty, one 'Pay total
   * price' button will be shown. If not empty, the first button must be a Pay button.
   * @param disableNotification Optional. Sends the message silently. Users will receive a notification with
   * no sound.
   * @param messageThreadId Optional. Unique identifier for the target message thread (topic) of the forum;
   * for forum supergroups only
   * @param maxTipAmount Optional. The maximum accepted amount for tips in the smallest units of the currency
   * (integer, not float/double). For example, for a maximum tip of US$ 1.45 pass max_tip_amount = 145. See
   * the exp parameter in https://core.telegram.org/bots/payments/currencies.json, it shows the number of
   * digits past the decimal point for each currency (2 for the majority of currencies). Defaults to 0
   * @param suggestedTipAmounts Optional. A JSON-serialized array of suggested amounts of tips in the smallest
   ...
   * maxTipAmount.
   * @param startParameter Optional. Unique deep-linking parameter. If left empty, forwarded copies of the
   * sent message will have a Pay button, allowing multiple users to pay directly from the forwarded message,
   * using the same invoice. If non-empty, forwarded copies of the sent message will have a URL button with a
   * deep link to the bot (instead of a Pay button), with the value used as the start parameter
   * @param protectContent Optional. Protects the contents of the sent message from forwarding and saving
   *
   * @return On success, the sent Message is returned.
   */
   ...
   * delivery cost, delivery tax, bonus, etc.)
   * @param maxTipAmount Optional. The maximum accepted amount for tips in the smallest units of the currency
   * (integer, not float/double). For example, for a maximum tip of US$ 1.45 pass maxTipAmount = 145. See the
   * exp parameter in https://core.telegram.org/bots/payments/currencies.json, it shows the number of digits
   * past the decimal point for each currency (2 for the majority of currencies). Defaults to 0
   * @param suggestedTipAmounts Optional. A JSON-serialized array of suggested amounts of tips in the smallest
   * units of the currency (integer, not float/double). At most 4 suggested tip amounts can be specified. The
   * suggested tip amounts must be positive, passed in a strictly increased order and must not exceed
   * maxTipAmount.
   * @param providerData Optional. JSON-serialized data about the invoice, which will be shared with the
   * payment provider. A detailed description of required fields should be provided by the payment provider.
   * @param photoUrl Optional. URL of the product photo for the invoice. Can be a photo of the goods or a
   ...
   * @param needShippingAddress Optional. Pass True, if you require the user's shipping address to complete
   * the order
   * @param sendPhoneNumberToProvider Optional. Pass True, if the user's phone number should be sent to the
   * provider
   * @param sendEmailToProvider Optional. Pass True, if the user's email address should be sent to the
   * provider
   * @param isFlexible Optional. Pass True, if the final price depends on the shipping method
   *
  ...
  dd::task<std::string> createInvoiceLink(
      ...
      std::int32_t photoWidth = 0, std::int32_t photoHeight = 0, bool needName = false,
      bool needPhoneNumber = false, bool needEmail = false, bool needShippingAddress = false,
      bool sendPhoneNumberToProvider = false, bool sendEmailToProvider = false,
      bool isFlexible = false) const;

  /**
   * @brief Use this method to reply to shipping queries.
   *
   * If you sent an invoice requesting a shipping address and the parameter isFlexible was specified, the Bot
   * API will send an Update with a shippingQuery field to the bot.
   *
   ...
   * @param shippingOptions Optional. Required if ok is True. A JSON-serialized array of available shipping
   * options.
   * @param errorMessage Optional. Required if ok is False. Error message in human readable form that explains
   ...
   * in the form of an Update with the field preCheckoutQuery. Note: The Bot API must receive an answer within
   * 10 seconds after the pre-checkout query was sent.
   *
   * @param preCheckoutQueryId Unique identifier for the query to be answered
   * @param ok Specify True if everything is alright (goods are available, etc.) and the bot is ready to
   * proceed with the order. Use False if there are any problems.
   * @param errorMessage Required if ok is False. Error message in human readable form that explains the
   ...
   * the field for which you returned the error must change). Use this if the data submitted by the user
   * doesn't satisfy the standards your service requires for any reason. For example, if a birthday date seems
   * invalid, a submitted document is blurry, a scan shows evidence of tampering, etc. Supply some details in
   ...
   * keyboard](https://core.telegram.org/bots/features#inline-keyboards). If empty, one 'Play gameTitle'
   * button will be shown. If not empty, the first button must launch the game. Not supported for messages
   ...
   *
   * Returns an error, if the new score is not greater than the user's current score in the chat and force is
   * False.
   *
   * @param userId User identifier
   * @param score New score, must be non-negative
   * @param force Optional. Pass True if the high score is allowed to decrease. This can be useful when fixing
   * mistakes or banning cheaters
   * @param disableEditMessage Optional. Pass True if the game message should not be automatically edited to
   ...
   * side. Will also return the top three users if the user and their neighbors are not among them. Please
