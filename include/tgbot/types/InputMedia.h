...
class InputMedia {
 public:
   ...
   * @brief File to send.
   *
   * Pass a fileId to send a file that exists on the Telegram servers (recommended), pass an HTTP URL for
   * Telegram to get a file from the Internet, or pass “attach://<file_attach_name>” to upload a new one using
   * multipart/form-data under <file_attach_name> name. https://core.telegram.org/bots/api#sending-files
   */
  std::string media;
