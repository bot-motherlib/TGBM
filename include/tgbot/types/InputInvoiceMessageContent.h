...
class InputInvoiceMessageContent : public InputMessageContent {
 public:
  ...
  /**
   * @brief Three-letter ISO 4217 currency code, see
   * https://core.telegram.org/bots/payments#supported-currencies
   */
  std::string currency;

  /**
   * @brief Price breakdown, a JSON-serialized list of components (e.g. product price, tax, discount, delivery
   * cost, delivery tax, bonus, etc.)
   */
  std::vector<LabeledPrice::Ptr> prices;

  /**
   * @brief Optional. The maximum accepted amount for tips in the smallest units of the currency (integer, not
   * float/double). For example, for a maximum tip of US$ 1.45 pass max_tip_amount = 145. See the exp
   * parameter in https://core.telegram.org/bots/payments/currencies.json, it shows the number of digits past
   * the decimal point for each currency (2 for the majority of currencies). Defaults to 0
   */
  std::int32_t maxTipAmount;

  /**
   * @brief Optional. A JSON-serialized array of suggested amounts of tip in the smallest units of the
   * currency (integer, not float/double). At most 4 suggested tip amounts can be specified. The suggested tip
   * amounts must be positive, passed in a strictly increased order and must not exceed maxTipAmount.
   */
  std::vector<std::int32_t> suggestedTipAmounts;

  /**
   * @brief Optional. A JSON-serialized object for data about the invoice, which will be shared with the
