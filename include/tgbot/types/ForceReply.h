 ...
 * sacrifice [privacy mode](https://core.telegram.org/bots/features#privacy-mode).
 *
 * Example: A [poll bot](https://t.me/PollBot) for groups runs in privacy mode (only receives commands,
 ...
 * - Guide the user through a step-by-step process. 'Please send me your question', 'Cool, now let's add the
 * first answer option', 'Great. Keep adding answer options, then send /done when you're ready'. The last
 * option is definitely more attractive. And if you use ForceReply in your bot's questions, it will receive
...
class ForceReply : public GenericReply {
 public:
  ...
  /**
   * @brief Shows reply interface to the user, as if they manually selected the bot's message and tapped
   * 'Reply'
   */
  bool forceReply;
